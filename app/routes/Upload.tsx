...
const Upload = () => {
    ...
    const handleAnalyze = async ({ companyName, jobTitle, jobDescription, file }: { companyName: string, jobTitle: string, jobDescription: string, file: File  }) => {
        ...
        if (!imageFile.file) {
            console.error("PDF conversion error:", imageFile.error); // ← ITO LAGAY MO
    ...
    const handleSubmsit = (e:FormEvent<HTMLFormElement>) =>{
        e.preventDefault();
    ...
    // @ts-ignore
    return(
        <main className="bg-[url('public/assets/public/images/bg-main.svg')] bg-cover min-h-screen ">
            <Navbar/>
            ...
            <section className="main-section">
                <div className="page-heading py-16">
                    ...
                    {isProcessing ? (
                        <>
                            <h2>{statusText}</h2>
                            <img src="public/assets/public/images/resume-scan.gif" className="w-full"></img>
                        </>
                    ):(
                        <h2> Drop your resume for an ATS score and improvement tips. </h2>
                    )}
                    {!isProcessing && (
                        <form id="upload-form" onSubmit={handleSubmsit} className="flex flex-col gap-4 mt-8">
